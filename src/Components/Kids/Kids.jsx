...
import { AiOutlineHeart, AiFillStar, AiOutlineStar, AiOutlineSearch, AiOutlineFilter, AiOutlineLeft, AiOutlineRight } from 'react-icons/ai';
...
const Kids = () => {
  ...
  const items = [
    { id: 1, image: baseball, name: "Baseball", description: "Cool baseball for kids.", price: "Rs.10", rating: 4 },
    { id: 2, image: cricketball, name: "Cricket Ball", description: "Best cricket ball for practice.", price: "Rs.12", rating: 4.5 },
    { id: 3, image: accessories1, name: "Accessory 1", description: "Stylish accessory for kids.", price: "Rs.5", rating: 3.5 },
    { id: 4, image: accessories2, name: "Accessory 2", description: "Fun accessory for playtime.", price: "Rs.8", rating: 4 },
  ];
  ...
  const handleAddToWishlist = (itemId) => {
    setWishlistItems((prevWishlistItems) =>
      prevWishlistItems.includes(itemId) ? prevWishlistItems : [...prevWishlistItems, itemId]
    );
    setShowPopup(true);
    ...
    <div className="kids">
      ...
      <div className="kids-page">
        <div className="filters">
          ...
          <AiOutlineFilter className="filter-icon" onClick={() => setShowFilters(!showFilters)} />
          {showFilters && (
        ...
        <div className="explore-collection">
          ...
          <div className="collection-images">
            <img src={collection1} alt="Collection 1" className="collection-image" />
            <img src={collection2} alt="Collection 2" className="collection-image" />
            <img src={collection3} alt="Collection 3" className="collection-image" />
          </div>
        </div>

        <div className="kids-gallery" style={{ backgroundImage: `url(${galleryImages[currentImageIndex]})` }}>
          <div className="gallery-heading">Kids Collection</div>
          <button className="shop-now-button">Shop Now</button>
          <div className="prev-icon arrow-icon" onClick={() => setCurrentImageIndex((currentImageIndex - 1 + galleryImages.length) % galleryImages.length)}>&#9664;</div>
          <div className="next-icon arrow-icon" onClick={() => setCurrentImageIndex((currentImageIndex + 1) % galleryImages.length)}>&#9654;</div>
        </div>
  ...
  {items.map((item) => (
    <div key={item.id} className="item-box">
      <div className="image-container">
        ...
        <AiOutlineHeart
          className={`wishlist-icon ${wishlistItems.includes(item.id) ? 'wishlist-active' : ''}`}
          onClick={() => handleAddToWishlist(item.id)}
        />
        <div className="star-rating">{renderStars(item.rating)}</div>
      </div>
      <div className="thumbnail-box">
        <img src={item.image} alt={`${item.name} thumbnail`} className="thumbnail-image" />
      </div>
